...
# Your original keyword list (kept so snippet still works)
EN_SENDER_KEYS = [
    r"\bSender\s*ID\b", r"\bSender\b", r"\bCustomer\s*ID\b", r"\bAccount\s*ID\b",
    r"\bFrom:\b", r"\bSender\s*Name\b", r"\bCustomer\s*Name\b"
]

# NEW: explicit "sender's name" style label patterns (colon optional)
...
def _best_name_like_line(cands: List[str]) -> Optional[str]:
    ...
    if not cleaned:
        return None
    # Score by alphabetic-ness and length; downweight short label-only lines
    scored = []
...
# --- your original "pick best from blocks" as a fallback ---
...
# --- NEW primary extractor (same-line → neighbor) ---
def extract_sender_name(page_text: str) -> Dict[str, str]:
    """
    1) same line after label (best)
...
def verify_name_on_page(page_text: str, customer_name: str, pass_threshold=85):
    ...
    ext = extract_sender_name(page_text)
    extracted = ext.get("extracted", "").strip()

    # If that failed, fall back to your old block-based pick
    if not extracted:
        blocks = find_sender_blocks(page_text)
        extracted = extract_best_name_candidate(blocks) or ""
    else:
        blocks = find_sender_blocks(page_text)  # still build snippet for audit
